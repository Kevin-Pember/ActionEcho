...
let actionRunnerScript = async (echoActions) => {
  let input = {
...
let logger = {
  indicator: {
    ...
    toggle: (way) => {
      if (logger.indicator.elem.className.includes("actionEchoRecordIcon")) {
        logger.indicator.elem.classList.add("actionEchoRecordIcon")
        logger.indicator.elem.innerHTML = `<svg style="height:60px; width: 60px;" viewBox="0 0 1200 1200" xmlns="http://www.w3.org/2000/svg">
    ...
    if (event.type == "click") {
      ...
      if ((event.metaKey || event.ctrlKey) && event.key.length == 1) {
        ...
        for (let action of logger.keyTable) {
          console.log(action[logger.browserOS])
